...
<Meta title="VTable/Introduction" />
...
- **headers**: [header]  
    - header: object literal containing  
        ...
        - _visibilist_ \<ƒunction\>  
            ...
            ```js
            visibility: {
                isVisible, setVisibility, header
            }
            ```  
            this is meant to allow to show a visibility toggler
        - _removedContent_ \<string\>  
            overriding value to show in the cells of the hidden headers; wins over _commonRemovedContent_ which wins over the default `-` value.  
...
- _header_ ⬆️   
 ...
 both object literals containing   
    ...
    the default content for _header_ and _footer_ cells is the _header.key_ and this can be easily changed specifying the omonimous function in the the header setting.  
...
- _RightMost_  \<ƒunction\>  
    ...
    ```
    this component is meant to return the content of a sticky header that will be rendered in the right.  
...
- _LeftMost_  \<ƒunction\>  
    ...
    row, rowIndex, type, from, to
    ```
    this component is meant to return the content of a sticky header that will be rendered in the left.  
    `type` is a value in `['header', 'body', 'footer']`
- _commonRemovedContent_ \<string\>  
    in case we use the _visibilist_ without spefifying on the header the _removedContent_, this will allow to overrive the default value '.'.  
- _defaultColumnWidth_ \<integer\>  
    each column have a default with of **150px**, this parameter allows to change that default value; still in each header is possible to set a specific value (will be always interpreted in pixel unit)



- _cls_ \<object literal\>  
    this is meant to allow to add some style both to some relevant elements and to the header,row, cell and cross on hover event:  
    ``` js
    highlight: {
...
- _events_ \<object literal\>
    ``` js
    // self explanatory, as second param will get a literal containing the row and header
